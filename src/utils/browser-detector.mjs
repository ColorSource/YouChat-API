...
export function detectBrowser(preferredBrowser = 'auto') {
    ...
    if (preferredBrowser === 'auto' || preferredBrowser === undefined) {
        if (browsers.chrome) return browsers.chrome;
        if (browsers.edge) return browsers.edge;
    ...
    console.error('Chrome or Edge was not found. Please install one of them.');
    process.exit(1);
}
...
function findWindowsBrowser(browserName) {
    ...
    const regQuery = (key) => {
        try {
            return execSync(`reg query "${key}" /ve 2>nul`, {stdio: ['ignore', 'pipe', 'pipe']})
                .toString()
                .trim()
                .split('\r\n')
                .pop()
